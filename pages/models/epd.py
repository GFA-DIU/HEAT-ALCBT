...
class EPDType(models.TextChoices):
    """Adopted from LCAx."""
    OFFICAL = "offical", "From a verified ILCD+EPD source"
...
class Impact(models.Model):
    impact_category = models.CharField(_("Impact Category"), max_length=20, choices=ImpactCategoryKey.choices)
    life_cycle_stage = models.CharField(_("Life Cycle Stage"), max_length=20, choices=LifeCycleStage.choices)
    unit = models.CharField(_("Impact Unit"), max_length=20, choices=Unit.choices, default=Unit.UNKNOWN)
...
class EPD(BaseModel, epdLCAx):
    ...
    def get_penrt_impact_sum(self):
       impacts = EPDImpact.objects.filter(epd=self, impact__impact_category="penrt", impact__life_cycle_stage="a1a3")
       return round(sum(impact.value for impact in impacts), 2)
