...
class TestHotWaterSystemViews:
    ...
    def test_save_multiple_hot_water_systems_success(self, client, user, building, hot_water_system_data):
        """Test saving multiple hot water systems at once."""
        client.force_login(user)

        system2_data = hot_water_system_data.copy()
        system2_data["type_of_hot_water_system"] = "boiler"
        system2_data["fuel_type"] = "diesel"
        ...
        response = client.post(
            reverse("hot_water_system_save_multiple"),
            data=json.dumps(payload),
            content_type="application/json"
        )
    ...
    def test_save_multiple_replaces_existing(self, client, user, building, hot_water_system_data, hot_water_system_model_data):
        """Test that saving multiple systems replaces existing ones."""
        client.force_login(user)
        ...
        # Replace with one new system
        payload = {
            "building_id": str(building.id),
            "hot_water_systems": [hot_water_system_data]
        }
        ...
        # Verify only one system exists now
        assert HotWaterSystem.objects.filter(building=building).count() == 1
