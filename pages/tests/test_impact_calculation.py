...
@pytest.mark.django_db
def test_mass_ton_component(create_epd, create_epd_impact, create_assembly, create_product):
    """Mass assembly + EPD declared per ton → resolved in kg then converted kg→ton (÷1000).

    100% of a 1-unit mass assembly = 1 kg = 0.001 ton; impact = 800 kgCO2e/ton × 0.001 = 0.8.
    """
    epd = create_epd("Rebar", Unit.TON, [])
    create_epd_impact(epd, Decimal("800"))  # per ton
    assembly = create_assembly(AssemblyDimension.MASS)
    p = create_product(assembly, epd, Decimal("100"), Unit.PERCENT)  # 100% share of mass
    assert _gwp(_calc(AssemblyDimension.MASS, p)) == pytest.approx(Decimal("0.8"))
